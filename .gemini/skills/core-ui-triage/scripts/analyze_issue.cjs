...
async function analyzeIssue(issueLink, maintainersList) {
  ...
  try {
    ...
    // 2. Fetch Timeline for cross references and labeling events
    const timelineFullCmd = `gh api repos/${repo}/issues/${issueNumber}/timeline --paginate --slurp -F per_page=100`;
    const timelineFullRaw = runCommand(timelineFullCmd);
    let lastNeedsInfoLabelDate = null;
    
    if (!timelineFullRaw.startsWith('{"error"')) {
        const fullTimeline = JSON.parse(timelineFullRaw);
        
        // Find merged PR cross-references
        result.cross_references = fullTimeline
          .filter(e => e.event === 'cross-referenced' && e.source && e.source.issue)
          .map(e => ({
            issue: e.source.issue.number,
            state: e.source.issue.state,
            is_pr: (e.source.issue.pull_request != null),
            is_merged: (e.source.issue.pull_request && e.source.issue.pull_request.merged_at != null)
          }));

        // Find the LATEST time 'status/needs-info' was added
        const labelEvents = fullTimeline.filter(e => e.event === 'labeled' && e.label && e.label.name === 'status/needs-info');
        if (labelEvents.length > 0) {
          lastNeedsInfoLabelDate = new Date(labelEvents[labelEvents.length - 1].created_at);
        }
    }

    // Check if a response was received AFTER the needs-info label was applied
    if (lastNeedsInfoLabelDate && result.labels.includes('status/needs-info')) {
       const newComments = issue.comments.filter(c => {
         const author = c.author.login;
         return author !== 'github-actions[bot]' && 
                author !== 'app/github-actions' &&
                !author.includes('bot') &&
                new Date(c.updatedAt) > lastNeedsInfoLabelDate;
       });
       if (newComments.length > 0) {
          result.needs_info_resolved = true;
       }
    }
