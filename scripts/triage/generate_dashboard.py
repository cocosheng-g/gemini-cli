...
def get_author_activity(pr):
    ...
    for commit in pr.get('commits', {}).get('nodes', []):
        c = commit.get('commit', {})
        committer = c.get('author', {})
        if committer and committer.get('user') and committer.get('user').get('login') == author:
...
def main():
    ...
    with open("triage/HELP_ISSUES_TRIAGE.md", "w") as f: f.write(md_rev)
    ...
    for login, data in sorted(member_stats.items(), key=lambda x: (-x[1]['weekly_closed'], -len(x[1]['open_queue']), x[1]['name'])):
