...
def main():
    ...
    md += "| :--- | :--- |\n"
    md += f"| 🆕 New Help Wanted Issues | **{new_issues}** |\n"
    md += f"| 🛠️ PRs Opened | **{new_prs}** |\n"
    md += f"| 🟣 PRs Merged | **{merged_prs}** |\n"
    md += f"| ⚪ PRs Closed (Unmerged) | **{unmerged_closed_prs}** |\n"
    md += f"| 🔄 Issue to PR Conversion Rate | **{conversion_rate:.1f}%** (Merged / Opened) |\n\n"
    
    md += "## ⏱️ Efficiency & Bottlenecks\n"
    md += "| Metric | Average |\n"
    md += "| :--- | :--- |\n"
    md += f"| ⚡ Time to First Review (TTFR) | **{avg_ttfr:.1f} hours** (Target: < 24h) |\n"
    md += f"| 🚢 Time to Merge (TTM) | **{avg_ttm:.1f} days** |\n\n"
    
    md += "## ❤️ Community Health\n"
    md += "| Metric | Rate |\n"
    md += "| :--- | :--- |\n"
    md += f"| 📉 Author Drop-off Rate | **{dropoff_rate:.1f}%** (Unmerged / Total Closed) |\n\n"
